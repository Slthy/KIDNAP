...
for round in $(seq 1 "$ROUNDS"); do
  ...
  REPLAY_TIMEOUT="$REPLAY_TIMEOUT" sudo --preserve-env=REPLAY_TIMEOUT "$ROOT_DIR/scripts/analyze_queue.py" "$PRESELECTED" --harness "$HARNESS" --reset-cmd "$RESET_CMD" --csv "$OUT/queue-growth.csv" > "$OUT/queue-analysis.json"
  NEXT="$OUT/seeds-next"
  "$ROOT_DIR/scripts/select_corpus.py" "$OUT/queue-growth.csv" "$PRESELECTED" "$NEXT" --top-k "$TOP_K"
  SEEDS="$NEXT"
done
