...
REPLAY_TIMEOUT="$REPLAY_TIMEOUT" TRACE_PROFILE="$TRACE_PROFILE" sudo --preserve-env=REPLAY_TIMEOUT,TRACE_PROFILE "$ROOT_DIR/scripts/analyze_queue.py" "$PRESELECTED" \
  --harness "$HARNESS" \
  --reset-cmd "$RESET_CMD" \
...
"$ROOT_DIR/scripts/select_corpus.py" \
  "$REPORT_DIR/queue-growth.csv" "$PRESELECTED" "$REPORT_DIR/seeds-next" \
  --top-k "$TOP_K" --min-score "$SELECT_MIN_SCORE" --strategy "$SELECT_STRATEGY"
